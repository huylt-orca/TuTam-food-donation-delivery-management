...
class ActivityRoleService {
  static const String url = "${AppConfig.SERVER}activity-roles";

  static List<ActivityRole> parserActivityRoleList(String responseBody) {
    var data = json.decode(responseBody);
    var list = data['data'] == null ? [] : data['data'] as List<dynamic>;
    List<ActivityRole> roles =
        list.map((model) => ActivityRole.fromJson(model)).toList();
    return roles;
  }

  static Future<List<ActivityRole>> fetchActivityRoleList(
    ...
    if (response.statusCode == 200) {
      return compute(parserActivityRoleList, response.body);
    } else {
