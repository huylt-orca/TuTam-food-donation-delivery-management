import 'dart:convert';

import 'package:flutter/foundation.dart';
import 'package:food_donation_delivery_app/app_config.dart';
import 'package:food_donation_delivery_app/exceptions/my_custom_exception.dart';
import 'package:food_donation_delivery_app/models/activity_type.dart';
import 'package:http/http.dart' as http;
class ActivityTypeService {
  static const String url = "${AppConfig.SERVER}activity-types";

  static List<ActivityType> parserActivityTypes(String responseBody){
    var data = json.decode(responseBody) ;
    var list = data['data']as List<dynamic>;
    List<ActivityType> notifications = list.map((model) => ActivityType.fromJson(model)).toList();
    return notifications;
  }

  static Future<List<ActivityType>> fetchActivityTypes() async{

    var headers = {
      'Content-type': 'application/json',
      'Accept': 'application/json',
      // 'Authorization': 'Bearer ${await StorageRepository.getAccessToken()}'
    };
    try {
      final response = await http.get(Uri.parse(url),headers: headers );
    if (response.statusCode ==200){
      return compute(parserActivityTypes,response.body);
    } else{
      throw MyCustomException.fromObject(json.decode(response.body));
    }
    } catch (error){
      if (error is MyCustomException) rethrow;
      throw Exception(error);
    }
    
  }
}