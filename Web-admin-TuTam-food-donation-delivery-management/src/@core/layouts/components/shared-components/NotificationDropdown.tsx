...
const NotificationDropdown = ({ userLogin }: { userLogin: UserModel }) => {
  ...
  const handleNotificationClick = async (notification: NotificationModel) => {
    ...
    handleDropdownClose()
    const url = await getUrl(notification)

    if ((notification.status === 'NEW' || notification.status === 0) && !!notification.id) {
  ...
  const getUrl = async (notification: NotificationModel) => {
    let key = notification.dataType
    if (typeof notification.dataType === 'string') {
      const indexOfS = Object.values(NotificationType).indexOf(notification.dataType as unknown as NotificationType)

      key = Object.keys(NotificationType)[indexOfS]
    }
    let url = ''
    ...
    switch (key) {
      ...
      case NotificationType.DELIVERY_REQUEST:
        url = '/van-chuyen/' + notification.dataId
        break
      case NotificationType.REPORTABLE_DELIVERY_REQUEST.toString():
      case NotificationType.REPORTABLE_DELIVERY_REQUEST:
        url = '/van-chuyen/' + notification.dataId
        break
      case NotificationType.DONATED_REQUEST.toString():
      case NotificationType.DONATED_REQUEST:
        url = '/quyen-gop/chi-tiet/' + notification.dataId
        break
      case NotificationType.AID_REQUEST.toString():
      case NotificationType.AID_REQUEST:
        url = '/danh-sach-yeu-cau-ho-tro/chi-tiet-yeu-cau-ho-tro/' + notification.dataId
        break
      ...
      case NotificationType.SCHEDULED_ROUTE:
        try {
          await DeliveryRequestAPI.getScheduledRouteDetail(notification.dataId as string)
    ...
    console.log({ url }, NotificationType.REPORTABLE_DELIVERY_REQUEST)

    return url
  }


  const handleGetNewNotification = async (newNoti: NotificationModel) => {
    const list = [newNoti, ...notifications]
    setNotificaitons([...list])
    setMessgaeUnseenCount(messgaeUnseenCount + 1)
    const url = await getUrl(newNoti)
  ...
  return (
    <Fragment>
      ...
      >
              ...
              >
                <Box sx={{ width: '100%', display: 'flex', alignItems: 'center' }}>
                  ...
                  <Box sx={{ mx: 4, flex: '1 1', display: 'flex', overflow: 'hidden', flexDirection: 'column' }}>
                    <MenuItemTitle color={item.status === 'NEW' || item.status === 0 ? 'black' : '#b7b7b7'}>
                      {item.name}
                    </MenuItemTitle>
        ...
        >
          <NotificationDialog handleNotificationClick={handleNotificationClick} readNotifications={readNotifications} />
        </MenuItem>
      </Menu>
    </Fragment>
  )
