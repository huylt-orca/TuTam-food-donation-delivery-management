...
import { compareWithCurrentTime, groupByMonth, sortScheduledTimes } from '../utils'
...
export default function DisplayScheduledTime(props: IDisplayScheduledTimeProps) {
  ...
  const [scheduleTimeGroupedByMonth, setScheduleTimeGroupedByMonth] = useState<Record<string, ScheduledTime[]>>({})
  ...
  return (
    <Grid container gap={3} justifyContent={'flex-start'} flexDirection={'row'}>
      {Object.entries(scheduleTimeGroupedByMonth).map(([key, value]) => {
        ...
        for (let index = 1; index <= moment(key).daysInMonth(); index++) {
          ...
          const isAfterNow = !compareWithCurrentTime(date?.day || '', date?.endTime || '')
