...
export default function RejectPostDialog(props: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res: any = await axiosClient.put(`/posts/admin/${props?.id}`, {
        isAccept: false,
        reason: reason
      })
      toast.success(res.message)
      props.setFilterObject({...props.filterObject, page: 1}) 
    } catch (error) {
      console.log(error)
