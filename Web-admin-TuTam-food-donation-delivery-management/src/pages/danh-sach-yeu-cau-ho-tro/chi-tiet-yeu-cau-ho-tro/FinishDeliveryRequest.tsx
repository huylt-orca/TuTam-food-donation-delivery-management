...
import { DeliveryRequestDetailModel, HistoryDeliveryRequestModel } from 'src/models/DeliveryRequest'
import { AidRequestAPI } from 'src/api-client/AidRequest'
import { PaginationModel, CommonRepsonseModel } from 'src/models/common/CommonResponseModel'
import { HistoryRecieveAidRequestModel, HistoryDetailRecieveAidRequestModel } from 'src/models/AidRequest'
...
export default function FinishDeliveryRequest(props: IFinishDeliveryRequestProps) {
  ...
  const [historyDeliveryRequest, setHistoryDeliveryRequest] = useState<HistoryDeliveryRequestModel[]>([])
  const [historyDeliveryDetial, setHistoryDeliveryDetail] = useState<DeliveryRequestDetailModel>(
  ...
  useEffect(() => {
    setPage(1)
    setHistoryDeliveryDetail(new DeliveryRequestDetailModel({}))
    ...
    if (currentId && page !== 0) {
      const fetchDataDetailSelfShipping = async () => {
        try {
          ...
          if (isSelfShipping) {
            setHistorySelfShipping(new CommonRepsonseModel<HistoryRecieveAidRequestModel[]>(response).data || [])
          } else {
            setHistoryDeliveryRequest(new CommonRepsonseModel<HistoryDeliveryRequestModel[]>(response).data || [])
          }
  ...
  const handleChange = async (id: string, isSelfShippingFlag: boolean) => {
    ...
    try {
      ...
      if (isSelfShippingFlag) {
        ...
        const response = await DeliveryRequestAPI.getDetialOfDeliveryRequest(id)
        setHistoryDeliveryDetail(
          new CommonRepsonseModel<DeliveryRequestDetailModel>(response).data ||
            new DeliveryRequestDetailModel({})
        )
        console.log(new CommonRepsonseModel<DeliveryRequestDetailModel>(response)?.data);
      }
              ...
              >
                ...
                >
                  <Box display={'flex'} gap={3}>
                    ...
                    >
                      <Typography className='text-white' fontWeight={500}>{`Nhận ${formateDateDDMMYYYYHHMM(
                        !!history.finishedDate ? history.finishedDate : history.exportedDate
                      )}`}</Typography>
                      <Typography
                ...
                >
                  ...
                  <Box display={'flex'} gap={2} flexWrap={'wrap'} alignContent={'stretch'}>
                    {historyDeliveryDetial.itemResponses?.map(item => {
                        return (
                          <Card
                            key={index}
                            sx={{
