...
const Auth: React.FC<AuthProps> = ({ children, role }) => {
  ...
  useEffect(() => {
    ...
    if (status === 'unauthenticated' && !role?.includes(KEY.ROLE.GUEST)) router.replace('/tinh-nguyen-vien/dang-nhap')
  }, [isUser, status, role, router, session])

  if (isUser) {
    if (role && !role.includes(session?.user?.role ?? '_')) {
      signOut({
        callbackUrl: '/401'
      })
    ...
    return <>{children}</>
  } else if (role?.includes(KEY.ROLE.GUEST)) {
    return <>{children}</>
  }
