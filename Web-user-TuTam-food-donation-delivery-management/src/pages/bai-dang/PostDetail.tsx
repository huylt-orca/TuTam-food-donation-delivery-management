...
export default function PostDetail({ userLogin, data, type }: any) {
	...
	const handlePushComment = async (id: any) => {
		try {
			...
			// setData([])
			setLoadComment({ page: 1, pageSize: 10 })
	...
	return (
		<Card sx={{ width: { xs: '100%', md: '90%' }, m: 'auto', mb: 5 }}>
			<CardHeader
				avatar={<Avatar sx={{ bgcolor: red[500] }} src={data?.createBy?.avatar} alt='avatar' />}
				title={data?.createBy?.fullName}
				subheader={
					data?.createdDate
						? format(new Date(data?.createdDate), 'dd/MM/yyyy hh:mm')
						: 'Chưa cập nhật'
				}
			/>
			{type === 'PRIVATE' && (
				<Box sx={{ mb: 3, ml: 18 }}>
					{data?.status === 'UNVERIFIED' && (
						<Chip label='Chưa xác thực' size='small' color='warning' />
					)}
					{data?.status === 'ACTIVE' && (
						<Chip label='Đã được phê duyệt' size='small' color='info' />
					)}
					{data?.status === 'INACTIVE' && (
						<Chip label='Ngưng hoạt động' size='small' color='error' />
					)}
					{data?.status === 'REJECT' && <Chip label='Đã bị từ chối' size='small' color='error' />}
				</Box>
			)}
			<Carousel
			...
			{data?.status === 'ACTIVE' && (
				<CardActions disableSpacing>
					{data?.status === 'ACTIVE' && !userLogin && (
						<Chip
							color='primary'
							variant='outlined'
							sx={{ p: 2 }}
							label='Vui lòng đăng nhập để có thể bình luận'
						/>
					)}
					{data?.status === 'ACTIVE' && userLogin && (
						<Button onClick={handleComment} startIcon={<MessageOutlinedIcon />}>
							Bình luận
						</Button>
					)}
