		...
		if (isConfirmed) {
			try {
				const response: any = await axiosClient.get(
					`/stock-updated-history-details/activity/${slug}/file?startDate=${filterObject.startDate}&endDate=${filterObject.endDate}`
				)
				if (response?.data) {
			...
			{data?.length === 0 && loading === false && (
				<Box sx={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
					<Typography variant='h5' sx={{ fontWeight: 700, textAlign: 'center' }}>
						Hiện không có dữ liệu sao kê cho hoạt động này
					</Typography>
				</Box>
			)}
			{data?.length > 0 &&
								...
								{d.type === 'IMPORT' && (
									<Stack direction={'column'}>
										<Typography>
											...
											{' '}cho <span style={{ fontWeight: 600, color: '#0288d1' }}>
												{d.pickUpPoint}
											</span>{' '}
										</Typography>
								...
								{d.type === 'EXPORT' && (
									<Stack direction={'column'}>
										<Typography>
											...
											, được quyên góp bởi{' '}
											<span style={{ fontWeight: 600, color: '#0288d1' }}>{d.donorName}</span> thông
											qua <span style={{ fontWeight: 600, color: '#0288d1' }}>{d.pickUpPoint}</span>{' '}
											đã giao cho{' '}
