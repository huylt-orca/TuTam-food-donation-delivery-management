...
export default function CharityUnitDetail({ charityUnit }: any) {
	...
	useEffect(() => {
		if (
			...
			charityUnit?.location !== undefined &&
			charityUnit?.location !== '_'
		) {
			...
			<Grid item container flexDirection={'row'} spacing={5}>
				...
				<Grid item xl lg md sm xs display={'flex'} gap={2} direction={'column'}>
					<Grid container spacing={3} sx={{ mt: 2 }}>
						...
						<Grid item>{CharityStatus[charityUnit?.status ?? '']}</Grid>
					</Grid>
